...
func NewService(name, version, typ, namespace string) *Service {
	xlog.Debugf("kubernetes default service: name: %s, version: %s", name, version)
...
func NewDeployment(name, version, typ, namespace string) *Deployment {
	xlog.Debugf("kubernetes default deployment: name: %s, version: %s", name, version)
	Labels := map[string]string{
		"name":    name,
		"version": version,
		"micro":   typ,
...
func NewClusterClient() *client {
	...
	if s == nil || !s.IsDir() {
		xerror.Panic(errors.New("service account not found"))
	}
