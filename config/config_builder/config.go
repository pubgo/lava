...
func (t *configImpl) initCfg() {
	...
	// 配置文件名字和类型
	v.SetConfigType(config.CfgType)
	v.SetConfigName(config.CfgName)
...
func (t *configImpl) initWithCfg(v *viper.Viper) bool {
	...
	xerror.Assert(pathutil.IsNotExist(config.CfgPath), "config file not found, path:%s", config.CfgPath)

	v.SetConfigFile(config.CfgPath)

	xerror.PanicF(v.ReadInConfig(), "config load error, path:%s", config.CfgPath)
