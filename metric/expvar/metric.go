...
// metric1 is an extended private interface with some additional internal
// methods used by timeseries. Counters, gauges and histograms implement it.
type metric1 interface {
	Metric
	Reset()
	Aggregate(roll int, samples []metric1)
}
...
func (c *counter) Aggregate(roll int, samples []metric1) {
	c.Reset()
...
func (g *gauge) Aggregate(roll int, samples []metric1) {
	g.Reset()
	g.Lock()
	defer g.Unlock()
...
func (h *histogram) Aggregate(roll int, samples []metric1) {
	h.Lock()
	defer h.Unlock()
...
func newTimeseries(builder func() metric1, frame string) *timeseries {
	var (
...
func newMetric(builder func() metric1, frames ...string) Metric {
	if len(frames) == 0 {
		return builder()
	}
	if len(frames) == 1 {
