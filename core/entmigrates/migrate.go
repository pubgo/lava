...
func (g *GoMigrate) Create(tables []*schema.Table) error {
	...
	now := time.Now()
	var name = fmt.Sprintf("sql%d", now.UnixNano())
	var filename = fmt.Sprintf("%s.sql", name)
	...
	var mmPkg = "github.com/pubgo/chameleon/internal/components/migrates"
...
`, name, name, fmt.Sprint(now.String(), " ", version.Version()), name))
	...
	err = os.WriteFile(filename, []byte(genFile.GoString()), 0644)
	if err != nil {
		return err
	}
	return nil
...
func (g *GoMigrate) tx(ctx context.Context) *Tx {
	tx := assert.Must1(g.db.DB().BeginTx(ctx, nil))
	return &Tx{sqlTx: &entsql.Tx{Conn: g.db.Conn, Tx: tx}, driver: g.db}
}
...
func (g *GoMigrate) insertMigration(ctx context.Context, m *Migration) error {
	...
	return withTx(ctx, tx, func(tx *Tx) error {
		_, err := tx.sqlTx.ExecContext(ctx, m.MigrationSql)
		if err != nil {
			return err
		}

		_, err = tx.sqlTx.ExecContext(ctx, sql, m.Name)
		if err != nil {
			return err
		}
		...
		return m.DataMigrate(ctx, g.builder, tx)
	})
}
...
func (g *GoMigrate) rollbackMigration(ctx context.Context, m *Migration) error {
	...
	return withTx(ctx, tx, func(tx *Tx) error {
		...
		return m.DataRollback(ctx, g.builder, tx)
	})
}
