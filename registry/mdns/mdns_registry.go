...
type DeregisterOption = registry.DeregisterOption
type Service = registry.Service
type Option = registry.Option
type GetOption = registry.GetOption
type Options = registry.Options
type Value = registry.Value
type WatchOptions = registry.WatchOptions
...
func (m *mdnsRegistry) Cfg() registry.Cfg {
	return m.cfg
}

func (m *mdnsRegistry) Init(cfg registry.Cfg) error {
	m.cfg = cfg
	return nil
}
...
func (m *mdnsRegistry) Register(service *Service, opts ...RegisterOption) error {
	...
	for _, node := range service.Nodes {
		...
		xlog.Debugf("[mdns] registry create new service with ip: %s for: %s", net.ParseIP(host).String(), host)
		...
		for {
			...
			case e := <-entries:
				...
				if len(e.AddrV4) > 0 {
					...
					xlog.Infof("[mdns]: invalid endpoint received: %v", e)
					continue
				}
...
func (m *mdnsRegistry) ListServices(opts ...registry.ListOption) ([]*Service, error) {
