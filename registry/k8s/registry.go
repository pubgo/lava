...
func (s *Registry) DeRegister(service *registry.Service, opt ...registry.DeRegOpt) error {
	return s.Register(&registry.Service{
...
func (s *Registry) ListServices(opt ...registry.ListOpt) ([]*registry.Service, error) {
	panic("implement me")
}
...
func (s *Registry) Deregister(service *registry.Service) error {
	return s.Register(&registry.Service{
		Metadata: map[string]string{},
	})
}

func (s *Registry) sendLatestInstances(name string, announcement chan []*registry.Service) {
	instances, err := s.GetService(name)
	if err != nil {
		panic(err)
	}
...
func (s *Registry) Watch(name string, opt ...registry.WatchOpt) (registry.Watcher, error) {
	...
	announcement := make(chan []*registry.Service, 1)
...
type Iterator struct {
	ch     chan []*registry.Service
	stopCh chan struct{}
}

// NewIterator is used to initialize Iterator
func NewIterator(channel chan []*registry.Service, stopCh chan struct{}) *Iterator {
	return &Iterator{
		ch:     channel,
		stopCh: stopCh,
	}
...
func (iter *Iterator) Next() ([]*registry.Service, error) {
	select {
...
func getServiceInstanceFromPod(pod *corev1.Pod) (*registry.Service, error) {
