...
import (
	"context"
	"github.com/pubgo/golug/golug_registry"
	"net"
	"strings"
	"sync"
	"time"
...
type mdnsTxt struct {
	Service   string
	Version   string
	Endpoints []*golug_registry.Endpoint
...
type mdnsRegistry struct {
	opts golug_registry.Options

	sync.Mutex
	services map[string][]*mdnsEntry
}

func newRegistry(opts ...golug_registry.Option) golug_registry.Registry {
	options := golug_registry.Options{
...
func (m *mdnsRegistry) Init(opts ...golug_registry.Option) error {
	for _, o := range opts {
		o(&m.opts)
	}
	return nil
...
func (m *mdnsRegistry) Options() golug_registry.Options {
	return m.opts
}

func (m *mdnsRegistry) Register(service *golug_registry.Service, opts ...golug_registry.RegisterOption) error {
	m.Lock()
	defer m.Unlock()
...
func (m *mdnsRegistry) Watch(opts ...golug_registry.WatchOption) (golug_registry.Watcher, error) {
	var wo golug_registry.WatchOptions
	for _, o := range opts {
		o(&wo)
	}
