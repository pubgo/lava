...
type mdnsWatcher struct {
	wo   golug_registry.WatchOptions
	ch   chan *mdns.ServiceEntry
	exit chan struct{}
}

func (m *mdnsWatcher) Next() (*golug_registry.Result, error) {
	for {
		select {
		case e := <-m.ch:
			...
			service := &golug_registry.Service{
				Name:      txt.Service,
				Version:   txt.Version,
			...
			service.Nodes = append(service.Nodes, &golug_registry.Node{
