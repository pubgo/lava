...
import (
	...
	"github.com/pubgo/lug/pkg/protoutil"
...
var (
	...
	grpccCall    = protoutil.Import("github.com/pubgo/lug/plugins/grpcc")
	xerrorCall   = protoutil.Import("github.com/pubgo/xerror")
	xgenCall     = protoutil.Import("github.com/pubgo/lug/xgen")
	fiberCall    = protoutil.Import("github.com/pubgo/lug/pkg/builder/fiber")
	gutilCall    = protoutil.Import("github.com/pubgo/lug/pkg/gutil")
	byteutilCall = protoutil.Import("github.com/pubgo/x/byteutil")
)

// generateFile generates a .lug.pb.go file containing service definitions.
...
func genRestRouter(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	for _, m := range service.Methods {
		...
		if method == http.MethodGet {
			...
			g.P(`	if `, stringsCall("Contains"), `(v, ",") && `, gutilCall("EqualFieldType"), `(req, `, reflectCall("Slice"), `, k) {`)
			...
			g.P(`	}`)
			g.P(`})`)
			g.P(`xerror.Panic(`, gutilCall("MapFormByTag"), `(req, data, "json"))`)
		} else {
