...
import (
	"fmt"
	protoutil2 "github.com/pubgo/lug/pkg/protoutil"
...
func genClient(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	g.P("}")
	g.P()
	g.P("fn(&", protoutil2.UnExport(service.GoName), "Client{c})")
	g.P("return")
	g.P("}")
	g.P("}")
}
...
func genRpcInfo(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service) {
	...
	for _, m := range service.Methods {
		...
		var defaultUrl bool
		hr, err := protoutil2.ExtractAPIOptions(m.Desc)
		if err != nil || hr == nil {
			defaultUrl = true
			isDefault = true
			hr = protoutil2.DefaultAPIOptions(string(file.GoPackageName), service.GoName, m.GoName)
		}
		method, path := protoutil2.ExtractHttpMethod(hr)
