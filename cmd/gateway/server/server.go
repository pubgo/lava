...
func New(ctx context.Context, opts ...Opt) (*gatewaySrv, error) {
	p := &gatewaySrv{}
	for _, o := range opts {
		if err := o(p); err != nil {
			return nil, err
...
func (p *gatewaySrv) Serve(ctx context.Context) error {
	var (
		m = &autocert.Manager{
	...
	p.mach.Go(func(routine machine.Routine) {
		...
		p.log.Debug("starting http server", zap.String("address", matcher.Addr().String()))
		if err := httpServer.Serve(matcher); err != nil && err != http.ErrServerClosed &&
			!strings.Contains(err.Error(), "mux: listener closed") {
			p.log.Error("http proxy failure", zap.Error(err))
		}
	})
	...
	p.mach.Go(func(routine machine.Routine) {
		...
		p.log.Debug("starting secure http server", zap.String("address", matcher.Addr().String()))
		if err := tlsHttpServer.Serve(matcher); err != nil &&
			err != http.ErrServerClosed &&
			!strings.Contains(err.Error(), "mux: listener closed") {
			p.log.Error("TLS http proxy failure", zap.Error(err))
		}
	})
	...
	p.mach.Go(func(routine machine.Routine) {
		...
		p.log.Debug("starting gRPC server", zap.String("address", matcher.Addr().String()))
		if err := gserver.Serve(matcher); err != nil && !strings.Contains(err.Error(), "mux: listener closed") {
			p.log.Error("gRPC proxy failure", zap.Error(err))
		}
	})
	shutdown = append(shutdown, func(ctx context.Context) {
	...
	p.mach.Go(func(routine machine.Routine) {
		...
		p.log.Debug("starting secure gRPC server", zap.String("address", matcher.Addr().String()))
		if err := tlsGserver.Serve(matcher); err != nil && !strings.Contains(err.Error(), "mux: listener closed") {
			p.log.Error("TLS gRPC proxy failure", zap.Error(err))
		}
	})
	shutdown = append(shutdown, func(ctx context.Context) {
		stopped := make(chan struct{}, 1)
	...
	p.mach.Go(func(routine machine.Routine) {
		if err := imux.Serve(); err != nil && !strings.Contains(err.Error(), "closed network connection") {
			p.log.Error("listener mux error", zap.Error(err))
		}
	})
	p.mach.Go(func(routine machine.Routine) {
	...
	p.log.Debug("shutdown signal received")
	...
	wg.Wait()
	p.mach.Wait()
	p.log.Debug("shutdown successful")
	return nil
}

// OverrideRoutes overrides the routes on the gatewaySrv. It is concurrency safe
func (p *gatewaySrv) OverrideRoutes(expressions []string) error {
...
func (p *gatewaySrv) gRPCDirector() proxy.StreamDirector {
		...
		if ok {
			if val, exists := md[":authority"]; exists && val[0] != "" {
				...
				defer func() {
					...
					p.log.Debug("proxied request", fields...)
				}()
				if target == "" {
...
func (p *gatewaySrv) httpDirector() func(r *http.Request) {
	return func(req *http.Request) {
		...
		if err != nil {
			p.log.Error("failed to find routing target", zap.Error(err))
			return
		}
		fields = append(fields, zap.String("target", target))
		if target == "" {
			p.log.Debug("empty routing target", fields...)
			return
		} else {
			u, err := url.Parse(target)
			if err != nil {
				fields = append(fields, zap.Error(err))
				p.log.Debug("failed to parse target", fields...)
				return
			}
...
func (p *gatewaySrv) getHttpRoute(req *http.Request) (string, error) {
