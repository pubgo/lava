...
func GetCmd() *cobra.Command {
	var cmd = &cobra.Command{
		Use: "grpcurl",
		Long: `Usage:
[flags] [address] [list|describe] [symbol]
...
`,
	...
	plaintext := flags.Bool("plaintext", false, prettify(`Use plain-text HTTP/2 when connecting to server (no TLS).`))
	insecure := flags.Bool("insecure", false, prettify(`Skip server certificate and domain verification. (NOT SECURE!) Not valid with -plaintext option.`))
	...
	flags.StringArrayVar(&addlHeaders, "H", nil, prettify(`
	...
	flags.StringArrayVar(&rpcHeaders, "rpc-header", nil, prettify(`
	...
	flags.StringArrayVar(&reflHeaders, "reflect-header", nil, prettify(`
		...
		method.`))
	flags.StringArrayVar(&protoset, "protoset", nil, prettify(`
		...
		-proto flags.`))
	flags.StringArrayVar(&protoFiles, "proto", nil, prettify(`
	...
	flags.StringArrayVar(&importPaths, "import-path", nil, prettify(`
	...
	flags.BoolVar(&reflection, "use-reflection", true, prettify(`
	...
	cmd.Run = func(cmd *cobra.Command, args []string) {
		...
		if !reflection && len(protoset) == 0 && len(protoFiles) == 0 {
