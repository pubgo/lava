...
func ExecuteFunc(template, startTag, endTag string, w io.Writer, f TagFunc) (int64, error) {
	s := convert.S2B(template)
	a := convert.S2B(startTag)
	b := convert.S2B(endTag)
...
func ExecuteFuncStringWithErr(template, startTag, endTag string, f TagFunc) (string, error) {
	if n := bytes.Index(convert.S2B(template), convert.S2B(startTag)); n < 0 {
		return template, nil
	}
...
func New(template, startTag, endTag string) *Template {
	t := assert.Must1(NewTemplate(template, startTag, endTag))
	return t
}
...
func (t *Template) Reset(template, startTag, endTag string) error {
	...
	assert.If(len(startTag) == 0, "startTag cannot be empty")
	assert.If(len(endTag) == 0, "endTag cannot be empty")
	...
	for {
		...
		t.tags = append(t.tags, convert.B2S(s[:n]))
		s = s[n+len(b):]
	}
...
func (t *Template) ExecuteFunc(w io.Writer, f TagFunc) (int64, error) {
	...
	n := len(t.texts) - 1
	if n == -1 {
		ni, err := w.Write(convert.S2B(t.template))
		return int64(ni), err
	}
...
func stdTagFunc(w io.Writer, tag string, m map[string]interface{}) (int, error) {
	...
	case TagFunc:
		return value(w, tag)
	default:
		return -1, errors.WrapCaller(fmt.Errorf("tag=%q contains unexpected value type=%#v. Expected []byte, string or TagFunc", tag, v))
	}
}
...
func keepUnknownTagFunc(w io.Writer, startTag, endTag, tag string, m map[string]interface{}) (int, error) {
	...
	if !ok {
		...
		if _, err := w.Write(convert.S2B(tag)); err != nil {
			return 0, err
		}
		if _, err := w.Write(convert.S2B(endTag)); err != nil {
			return 0, err
		}
